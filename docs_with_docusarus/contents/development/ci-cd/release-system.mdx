...
:::info Reusable Workflow Architecture

The release workflows in this project leverage **centralized reusable workflows** from the Template-Python-UV-Project repository. This architecture provides:

- **Centralized Logic**: Core release logic maintained in the template repository
- **Consistency**: All projects using the template benefit from the same battle-tested workflows
- **Easy Updates**: Workflow improvements automatically available to all child projects
- **Reduced Maintenance**: No need to duplicate complex workflow logic in each project

**Reusable Workflows Used:**
- [`rw_release_complete.yaml`](https://github.com/Chisanan232/Template-Python-UV-Project/blob/master/.github/workflows/rw_release_complete.yaml) - Production release logic
- [`rw_release_validation_complete.yaml`](https://github.com/Chisanan232/Template-Python-UV-Project/blob/master/.github/workflows/rw_release_validation_complete.yaml) - Validation logic
- [`rw_release_staging_complete.yaml`](https://github.com/Chisanan232/Template-Python-UV-Project/blob/master/.github/workflows/rw_release_staging_complete.yaml) - Staging release logic
...
- 📖 **Documentation**: [Template Project Documentation](https://chisanan232.github.io/Template-Python-UV-Project)
- 🔄 **Workflows Directory**: [`.github/workflows/`](https://github.com/Chisanan232/Template-Python-UV-Project/tree/master/.github/workflows)
...
graph TD
    ...
    B --> B1[Check Dockerfile]
    B1 -->|Exists| B2[Enable Docker Jobs]
    B1 -->|Missing| B3[Skip Docker Jobs]
...
- Sets `has_dockerfile` output for downstream Docker jobs
- Enables conditional execution of all Docker-related processes
...
2. **Docker Image** (`GHCR` & `DockerHub`) - **Config Enhanced**:
   - **Automatic Dockerfile Detection**: 🆕 Detects Dockerfile presence; gracefully skips all Docker processes if not found
   - **Conditional Execution**: Docker jobs only run when both a Dockerfile exists AND configuration allows it
   ...
   - **Dual Registry Publishing**: Publishes to both registries simultaneously (when Dockerfile exists)
   ...
   - **Smart Summaries**: Workflow outputs clearly indicate Docker skipped vs published
...
graph TD
    ...
    B --> B1[Check Dockerfile]
    B1 -->|Exists| B2[Run Docker Validation]
    B1 -->|Missing| B3[Skip Docker Validation]
    
    C --> D[Python Build Check]
    B2 --> E[Docker Build & Test - Config Based]
    B3 -.->|Skipped| E
    C --> F[Docs Build Test - Config Based]
    E --> G[Supply Chain Security]
    ...
    O -->|No Dockerfile| P[Docker Skipped Status]
    O -->|Has Dockerfile| Q[Docker Validated Status]
```
...
```
- Detects Dockerfile presence in repository root
- Determines whether Docker validation should run
- Outputs boolean flag for conditional job execution
- Enables graceful skipping for Python-only projects

**Step 3: Release Intent Validation**
```yaml
intent-parse:
  uses: Chisanan232/Template-Python-UV-Project/.github/workflows/rw_parse_release_intent.yaml@master
  needs: config
...
- **Automatic Dockerfile Detection**: 🆕 Checks for Dockerfile presence; skips Docker validation if not found
- **Conditional Docker Validation**: Docker build tests and security scans only run when Dockerfile exists
...
- **Flexible Validation**: Works for both Docker-based and Python-only projects
...
graph TD
    ...
    B --> B1[Check Dockerfile]
    B1 -->|Exists| B2[Build Docker RC Images]
    B1 -->|Missing| B3[Skip Docker RC]
    ...
    B2 --> E
    E --> H1[DockerHub RC - Config Based]
    E --> H2[GHCR RC - Config Based]
    B3 -.->|Skipped| E
...
```
- Checks for Dockerfile in repository root
- Controls Docker RC image builds for both DockerHub and GHCR
- Enables Python-only projects to skip Docker staging
- Provides clear feedback in staging summary
...
2. **Docker Staging (DockerHub & GHCR)** - **Config Enhanced**: 
   - **Automatic Dockerfile Detection**: 🆕 RC images only built if Dockerfile exists in repository
   - Docker images with `-rc` tags using `${{ needs.config.outputs.docker_registry_dockerhub }}` and `${{ needs.config.outputs.docker_registry_ghcr }}`
   ...
   - Gracefully skips if no Dockerfile present
...
✅ **Flexible**: 🆕 Automatic Dockerfile detection - works for both Docker-based and Python-only projects  
...
**New Feature**: All release workflows now automatically detect the presence of a `Dockerfile` in the repository root. If no Dockerfile is found, all Docker-related processes are gracefully skipped, regardless of the `docker` configuration setting.

**Smart Behavior:**
- ✅ **Dockerfile exists**: Docker processes run based on `docker` configuration (`auto`, `force`, `skip`)
- ⚠️ **No Dockerfile**: Docker processes are automatically skipped with clear logging
- 📋 **Summary output**: Workflow summaries clearly indicate when Docker was skipped due to missing Dockerfile

**Affected Workflows:**
- **Production Release** (`release.yml`): Skips Docker Hub and GHCR publishing
- **Validation** (`release-validate.yml`): Skips Docker build validation and security scanning
- **Staging** (`release-staging.yml`): Skips Docker Hub and GHCR RC image publishing

**Example Workflow Output:**
```
=== Release Summary ===
Dockerfile Check: ⚠️ Not Found (Docker skipped)
Python Package: ✅ Published to PyPI
DockerHub Build: ⏭️ Skipped (no Dockerfile)
GHCR Build: ⏭️ Skipped (no Dockerfile)
Documentation: ✅ Versioned
```

This allows the same template workflows to work seamlessly for Python-only projects without requiring any configuration changes.
:::
...
| Configuration | Dockerfile Exists | Changed Files | Behavior |
|---------------|-------------------|---------------|----------|
| `python: auto` | N/A | Python files changed | ✅ Publishes to PyPI |
| `python: auto` | N/A | No Python files changed | ⏭️ Skips publishing |
| `python: force` | N/A | Any/no changes | ✅ Always publishes |
| `python: skip` | N/A | Any/no changes | ⏭️ Always skips |
| `docker: auto` | ✅ Yes | Docker files changed | ✅ Publishes to registries |
| `docker: auto` | ✅ Yes | No Docker files changed | ⏭️ Skips publishing |
| `docker: auto` | ❌ No | Any/no changes | ⏭️ Skips (no Dockerfile) |
| `docker: force` | ✅ Yes | Any/no changes | ✅ Always publishes |
| `docker: force` | ❌ No | Any/no changes | ⏭️ Skips (no Dockerfile) |
| `docker: skip` | Any | Any/no changes | ⏭️ Always skips |
| `docs.mode: auto, strategy: changed` | N/A | Docs files changed | ✅ Versions changed sections |
| `docs.mode: auto, strategy: always` | N/A | Any changes | ✅ Versions all sections |
| `docs.mode: force` | N/A | Any/no changes | ✅ Always versions |
