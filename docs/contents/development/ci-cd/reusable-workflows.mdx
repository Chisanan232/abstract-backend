...
The test-python-uv-project-template CI/CD system is built on a foundation of reusable workflows that provide standardized operations across different contexts. This modular architecture promotes consistency, maintainability, and follows the DRY (Don't Repeat Yourself) principle.
...
with:
  operation: 'build'
  image-name: 'test-python-uv-project-template'
...
Reusable workflows support sophisticated conditional execution based on configuration, change detection, and environment checks:

```yaml
jobs:
  # Check for Dockerfile (New Feature)
  check-dockerfile:
  ...
  # Conditional Docker publishing with Dockerfile detection
  publish_docker:
    needs: [parse_intent, check-dockerfile, build_docker]
    if: |
      needs.check-dockerfile.outputs.has_dockerfile == 'true' &&
      (needs.parse_intent.outputs.docker == 'force' ||
       (needs.parse_intent.outputs.docker == 'auto' && needs.detect_changes.outputs.docker == 'true'))
    uses: ./.github/workflows/rw_docker_operations.yaml
    with:
      operation: 'push'
      registry: 'ghcr.io'
...
- **Dockerfile Detection** 🆕: Docker jobs only run when `Dockerfile` exists in repository root
- **Configuration-Based**: Jobs respect `force`, `auto`, or `skip` settings from intent
- **Change Detection**: Auto mode considers file changes for smart execution
- **Multi-Condition**: Combines environment checks with configuration for robust control
