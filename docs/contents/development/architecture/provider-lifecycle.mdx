...
- **Confirm message semantics**: Align on what your `MessageQueueMessage` will contain beyond the payload (metadata, IDs, timestamps).
...
- **Contract tests**: Reuse the project’s suites under `test/contract_test/backends/message_queue/` to confirm protocol compliance.
