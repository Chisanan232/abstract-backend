...
This page explains the default layout provided by the Abstract Backend so you can quickly understand where to place code, configuration, and automation for your own project.
...
- Extend with more directories (e.g., `contract_test/`) if your project requires broader coverage.
- Use the provided pytest plugins (coverage, asyncio, reruns) via `pyproject.toml` dependencies.
- Tests are picked up automatically by `ci.yaml`.
...
- **`release.yml`** – Intent-driven release process (Python package + docs).
...
- **Supporting workflows** – docs auto-merge, docs build check, etc.
