...
sequenceDiagram
    ...
    Loader->>Entry: fetch group "abe.backends.message_queue"
    ...
    Loader-->>Dev: MessageQueueBackend

    Dev->>Provider: publish(key, payload)
    loop message stream
        Provider-->>Consumer: yield MessageQueueMessage via consume()
...
- `load_backend()` in `abe/backends/message_queue/loader.py` reads the `abe.backends.message_queue` entry-point group via `importlib.metadata.entry_points()`.
...
- Application code calls `backend.publish(key, payload)` where `key` is a `MessageQueueKey` and `payload` is a `MessageQueuePayload` (both from `abe/types.py`).
...
- Each yielded `MessageQueueMessage` is passed to your handler coroutine. The helper logs unexpected exceptions but keeps the loop alive unless a fatal error occurs.
...
- Contract tests under `test/contract_test/backends/message_queue/` assert that new implementations honour cancellation handling and message semantics.
...
- [MessageQueueBackend protocol reference](/docs/next/api-references/message-queue-backend)
