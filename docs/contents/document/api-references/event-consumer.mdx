...
- `EventConsumer` lives in `abe/backends/queue/base/consumer.py` and is defined as a [PEP 544](https://peps.python.org/pep-0544/) protocol.
- `AsyncLoopConsumer` in `abe/backends/queue/consumer.py` is the default implementation that wraps a `QueueBackend` and repeatedly invokes your handler coroutine.

## `EventConsumer` protocol

```python title="abe/backends/queue/base/consumer.py"
...
```python title="abe/backends/queue/consumer.py"
...
- **`backend`**: The `QueueBackend` instance returned by `load_backend()` or injected directly.
