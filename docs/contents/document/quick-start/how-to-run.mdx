...
- `publish()` accepts a routing key plus a JSON-serialisable payload (`QueuePayload`).
...
- `AsyncLoopConsumer` lives in `abe/backends/queue/consumer.py` and works with any provider that implements `QueueBackend.consume()`.
